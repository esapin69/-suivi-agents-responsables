/**
 * Module Évaluations 2026 — à ajouter comme nouveau fichier EVALUATIONS.gs
 * dans le même projet Apps Script que Code.gs.
 * Prérequis : Services > + > Google Drive API (service avancé) activé.
 */
...
function saveEvaluationDraft_(p) {
  ...
  try {
    ...
    let existing = normalized.id_evaluation ? rows.find(x => x.id_evaluation === normalized.id_evaluation) : null;
...
function normalizeEvaluationPayload_(p, finalizing) {
  const result = {
    id_evaluation: boundedText_(p.id_evaluation,100,'ID_EVALUATION_TROP_LONG'), id_agent: boundedText_(p.id_agent,100,'ID_AGENT_TROP_LONG'), grade: boundedText_(p.grade,100,'GRADE_TROP_LONG'), service: boundedText_(p.service,200,'SERVICE_TROP_LONG'),
    date_evaluation: clean_(p.date_evaluation), evaluateur: boundedText_(p.evaluateur,250,'EVALUATEUR_TROP_LONG'), lyon_le: clean_(p.lyon_le), garder_agent: clean_(p.garder_agent).toUpperCase(),
    observations_1: boundedText_(p.observations_1,10000,'OBSERVATION_TROP_LONGUE'), observations_2: boundedText_(p.observations_2,10000,'OBSERVATION_TROP_LONGUE'), observations_3: boundedText_(p.observations_3,10000,'OBSERVATION_TROP_LONGUE'), observations_4: boundedText_(p.observations_4,10000,'OBSERVATION_TROP_LONGUE'), observations_5: boundedText_(p.observations_5,10000,'OBSERVATION_TROP_LONGUE'), observations_generales: boundedText_(p.observations_generales,10000,'OBSERVATION_TROP_LONGUE'),
...
function normalizeSignatures_(input) {
  ...
  ['agent','responsable','direction'].forEach(name=>{
    const value=clean_(source[name]);
    if(!value)return;
...
function evaluationRecordToRow_(r) {
  ...
  return values.map(value=>value instanceof Date||typeof value==='number'?value:safeSheetText_(value));
}

function evaluationRows_() {
  const sheet = evaluationSheet_(), last = sheet.getLastRow();
  if (last < 2) return [];
...
function generateOfficialEvaluation_(r) {
  ...
  const copied=convertWordTemplateToGoogleDoc_(safeName);
  const googleDocFile=DriveApp.getFileById(copied.id);
  const doc=openConvertedDocument_(copied.id);
  fillOfficialDocument_(doc,r);
  doc.saveAndClose();
  Utilities.sleep(800);
  const pdfBlob=googleDocFile.getAs(MimeType.PDF).setName(safeName+'.pdf');
...
function fillOfficialDocument_(doc,r) {
  ...
  fillTableRatings_(body,r.criteres);
  fillObservations_(body,[r.observations_1,r.observations_2,r.observations_3,r.observations_4,r.observations_5,r.observations_generales]);
  replaceEverywhere_(doc,/OUI\s*[☐☒]?\s*NON\s*[☐☒]?/i,(r.garder_agent==='OUI'?'OUI ☒   NON ☐':'OUI ☐   NON ☒'));
  insertSignatures_(body,r.signatures||{});
}

function insertSignatures_(body,signatures) {
  for(let i=0;i<body.getNumChildren();i++){
    const child=body.getChild(i);
    if(child.getType()!==DocumentApp.ElementType.PARAGRAPH)continue;
    const paragraph=child.asParagraph(),text=normalize_(paragraph.getText());
    if(text.indexOf('SIGNATUREDELAGENT')>=0||text.indexOf('SIGNATUREDURESPONSABLE')>=0){
      if(signatures.agent||signatures.responsable){
        paragraph.appendText('\n');
        if(signatures.agent)appendSignatureImage_(paragraph,signatures.agent,'agent');
        if(signatures.agent&&signatures.responsable)paragraph.appendText('\t\t');
        if(signatures.responsable)appendSignatureImage_(paragraph,signatures.responsable,'responsable');
      }
    }else if(text.indexOf('DIRECTIONDESSOINS')>=0&&signatures.direction){
      paragraph.appendText('\n');
      appendSignatureImage_(paragraph,signatures.direction,'direction');
    }
  }
}
...
function fillTableRatings_(body,criteria) {
  body.getTables().forEach(table=>{for(let i=0;i<table.getNumRows();i++){const row=table.getRow(i);for(let j=0;j<row.getNumCells();j++){const label=clean_(row.getCell(j).getText());const criterion=EVAL_CRITERIA.find(c=>normalize_(c)===normalize_(label));if(!criterion)continue;const selected=EVAL_LEVELS.indexOf(criteria[criterion]);for(let k=0;k<EVAL_LEVELS.length && j+1+k<row.getNumCells();k++)row.getCell(j+1+k).setText(k===selected?'X':'');}}});
}
function fillObservations_(body,values) {
  let cursor=0;
  for(let i=0;i<body.getNumChildren()&&cursor<values.length;i++){
    const child=body.getChild(i);
    if(child.getType()!==DocumentApp.ElementType.PARAGRAPH)continue;
    const paragraph=child.asParagraph(),label=normalize_(paragraph.getText());
    if(label.indexOf('OBSERVATION')!==0)continue;
    const value=clean_(values[cursor++]);
    paragraph.setText(label.indexOf('GENERALES')>=0?'OBSERVATIONS GENERALES :':'OBSERVATIONS :');
    if(value)paragraph.appendText('\n'+value);
  }
}

function documentSections_(doc) {
  return [doc.getBody(),doc.getHeader(),doc.getFooter()].filter(Boolean);
}
...
function setDocumentLine_(doc,token,replacement) {
  ...
  for(const section of documentSections_(doc)){
    for(let i=0;i<section.getNumChildren();i++){
      ...
      if(normalize_(paragraph.getText()).indexOf(wanted)===0){paragraph.setText(replacement);return true;}
    }
  }
  return false;
}
...
function replaceDateInDocument_(doc,dateText) {
  for(const section of documentSections_(doc)){
    for(let i=0;i<section.getNumChildren();i++){
      const child=section.getChild(i);
      if(child.getType()!==DocumentApp.ElementType.PARAGRAPH)continue;
      const paragraph=child.asParagraph(),text=paragraph.getText();
      if(normalize_(text).indexOf('DATE')<0)continue;
      paragraph.setText(text.replace(/DATE\s*:[^\n\r]*/i,'DATE : '+dateText));
      return true;
    }
  }
  return false;
...
function replaceEverywhere_(doc,regex,replacement) {
  ...
  [doc.getBody(),doc.getHeader(),doc.getFooter()].filter(Boolean).forEach(section=>{try{section.replaceText(pattern,safe);}catch(_) {}});
}
...
function openConvertedDocument_(fileId) {
  ...
  for (let attempt=0; attempt<6; attempt++) {
    ...
    try { return DocumentApp.openById(fileId); }
...
function convertWordTemplateToGoogleDoc_(name) {
  ...
  try {
    return Drive.Files.create(
      {name:name,mimeType:'application/vnd.google-apps.document',parents:[EVAL_CONFIG.DEST_FOLDER_ID]},
      template.getBlob(),
      {fields:'id,name,mimeType'}
    );
  } catch (err) {
    throw new Error('CONVERSION_MODELE_WORD_ECHOUEE: '+String(err&&err.message||err));
  }
}

/** Test non destructif de configuration. À exécuter une fois depuis l’éditeur. */
function testEvaluationConfiguration() {
  const sh=evaluationSheet_();
  const template=DriveApp.getFileById(EVAL_CONFIG.TEMPLATE_ID);
  ...
  try {
    const copiedDoc=openConvertedDocument_(copy.id);
    copiedDoc.getBody().getText();
