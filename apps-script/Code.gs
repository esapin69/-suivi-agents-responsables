...
function getApiKey_() {
  const raw = PropertiesService.getScriptProperties().getProperty('API_KEY');
...
function accessForAction_(action) {
  const map = {
    ...
    listDirectory: 'suivi_des_agents',
...
function publicAccessUser_(row) {
  const isAdmin =
    normalize_(row.nom) === 'SAPIN' &&
    normalize_(row.prenom) === 'EDDY';
...
function submitSituation_(p, principal) {
  ...
  try {
    ...
    const situationId =
      'SIT-' + Utilities.getUuid().replace(/-/g, '').slice(0, 12).toUpperCase();
    const now = new Date();
    ...
    if (
      clean_(verified[0]) !== situationId ||
      clean_(verified[1]) !== agent.id_agent
    ) {
    ...
    return {
      ...
      responsable,
      cree_le: formatDateTimeForClient_(now)
    };
  } finally {
    lock.releaseLock();
  }
...
function testSecurityConfiguration() {
  const authorized = accessRows_()
    .filter(isAuthorizedAccessRow_)
    .map(row => ({
      nom: row.nom,
      prenom: row.prenom,
      poste: row.poste,
      is_admin: publicAccessUser_(row).is_admin
    }));
  ...
  try {
    ...
    if (matricule) {
      const same = rows.find(
        r => normalize_(r[COL.MATRICULE - 1]) === normalize_(matricule)
      );
      if (same) {
        return {
          ok:false,
          code:'MATRICULE_EXISTANT',
    ...
    const copy = DriveApp.getFileById(CONFIG.TEMPLATE_ID).makeCopy(
      `${nom} ${prenom} - BROUILLON`,
      DriveApp.getFolderById(CONFIG.DEST_FOLDER_ID)
    );

    try {
      fillIdentitySheet_(copy.getId(), {
        nom,
        prenom,
        telephone,
        matricule,
      ...
      if (
        !verified ||
        verified.id_agent !== id ||
        verified.fichier_brouillon_id !== copy.getId()
      ) {
...
function updateAgent_(p) {
  ...
  try {
    ...
    const telephone = boundedText_(
      p.telephone !== undefined ? p.telephone : current[COL.TELEPHONE - 1],
      50,
      'TELEPHONE_TROP_LONG'
    );
    const matricule = boundedText_(
      p.matricule !== undefined ? p.matricule : current[COL.MATRICULE - 1],
      100,
      'MATRICULE_TROP_LONG'
    );
    const dateArrivee =
      p.date_arrivee !== undefined
        ? clean_(p.date_arrivee)
        : clean_(current[COL.DATE_ARRIVEE - 1]);
    const experiences = boundedText_(
      p.experiences !== undefined ? p.experiences : current[COL.EXPERIENCES - 1],
      10000,
      'EXPERIENCES_TROP_LONGUES'
    );
    ...
    if (matricule) {
      const duplicate = values.find(
        (r, i) =>
          i > 0 &&
          i !== rowIndex &&
          normalize_(r[COL.MATRICULE - 1]) === normalize_(matricule)
      );
      if (duplicate) {
        return {
          ok:false,
    ...
    sheet.getRange(rowIndex + 1, COL.NOM, 1, 7)
      .setValues([[
        nom,
        prenom,
        telephone,
    ...
    return {
      ok:true,
      verified:true,
      message:'Modifications enregistrées et vérifiées.',
      agent:getAgent_(id)
    };
  } finally {
    lock.releaseLock();
...
function fillIdentitySheet_(spreadsheetId, a) {
  ...
  if (a.dateArrivee) {
    ...
    if (parts.length === 3) {
      ...
      sh.getRange('C9').setValue(a.dateArrivee);
    }
  } else {
...
function getFirstDay_(id) {
  ...
  const sh = SpreadsheetApp
    .openById(agent.fichier_brouillon_id)
    .getSheetByName('1er jour');
  ...
  const items = roles.map((r, i) => ({
    row:i + 5,
    cadre:isChecked_(r[0]),
    chef:isChecked_(r[1]),
    tuteur:isChecked_(r[2]),
  ...
  return {
    ...
    date_validation:
      agent.premier_jour_date ||
      extractDateFromValidation_(clean_(sh.getRange('C30').getDisplayValue())),
...
function saveFirstDay_(p) {
  ...
  try {
    ...
    if (incoming.length !== 24) {
      throw new Error('PREMIER_JOUR_24_ETAPES_REQUISES');
    }

    const normalized = incoming.map((x, i) => ({
      row:i + 5,
      cadre:Boolean(x.cadre),
      chef:Boolean(x.chef),
      tuteur:Boolean(x.tuteur),
      observation:boundedText_(x.observation, 4000, 'OBSERVATION_TROP_LONGUE')
    }));

    const completed = normalized.filter(
      x => x.cadre || x.chef || x.tuteur
    ).length;

    const wantsFinalize =
      p.finalize === true || String(p.finalize || '') === '1';

    const currentStatus =
      clean_(agentRow[COL.PREMIER_JOUR_STATUT - 1]) || 'A_FAIRE';
    ...
    sh.getRange('E5:E28').setValues(
      normalized.map(x => [safeSheetText_(x.observation)])
    );
    ...
    if (wantsFinalize) {
      newStatus = 'TERMINE';
      sh.getRange('A30').setValue(
        'CHEF D’ÉQUIPE – NOM ET PRÉNOM : ' + chefNom
      );
      sh.getRange('C30').setValue(
        'DATE : ' + displayDateFr_(dateValidation)
      );
      ...
      newStatus =
        completed > 0 || normalized.some(x => x.observation)
          ? 'BROUILLON'
          : 'A_FAIRE';
    }
    ...
    if (newStatus === 'TERMINE' && dateValidation) {
      const d = parseIsoDate_(dateValidation);
      sheet.getRange(rowNumber, COL.PREMIER_JOUR_DATE)
        .setValue(d || dateValidation);
      if (d) {
        sheet.getRange(rowNumber, COL.PREMIER_JOUR_DATE)
    ...
    sheet.getRange(rowNumber, COL.PREMIER_JOUR_MODIFIE_LE)
      .setValue(new Date());

    SpreadsheetApp.flush();

    const verified = getFirstDay_(id);
    if (!verified || verified.completed !== completed) {
      throw new Error('VERIFICATION_PREMIER_JOUR_ECHOUEE');
    }
    ...
    return {
      ...
      message:
        newStatus === 'TERMINE'
          ? '1er jour terminé, enregistré et vérifié.'
...
function listDirectory_() {
  ...
  tracked.forEach(a => {
    byName[normalize_(a.nom) + '|' + normalize_(a.prenom)] = a;
  });

  const lastRow = sh.getLastRow();
  if (lastRow < 2) return [];

  return sh.getRange(2, 1, lastRow - 1, 3)
    .getDisplayValues()
    .filter(r => clean_(r[0]) && clean_(r[1]))
    .map(r => {
      const existing =
        byName[normalize_(r[0]) + '|' + normalize_(r[1])] || null;
...
function findDirectoryAgent_(nom, prenom) {
  ...
  const row = sh.getRange(2, 1, lastRow - 1, 3)
    .getDisplayValues()
    .find(
      r =>
        normalize_(r[0]) + '|' + normalize_(r[1]) === target
    );
...
function getAgent_(id) {
  ...
  const r = sheet.getRange(2, 1, lastRow - 1, 15)
    .getValues()
  ...
  return {
    id_agent:clean_(r[COL.ID - 1]),
    nom:clean_(r[COL.NOM - 1]),
    prenom:clean_(r[COL.PRENOM - 1]),
    telephone:clean_(r[COL.TELEPHONE - 1]),
    matricule:clean_(r[COL.MATRICULE - 1]),
    date_arrivee:formatDateForClient_(r[COL.DATE_ARRIVEE - 1]),
    experiences:clean_(r[COL.EXPERIENCES - 1]),
    verification:clean_(r[COL.VERIFICATION - 1]),
    fichier_brouillon_id:clean_(r[COL.FILE_ID - 1]),
    fichier_brouillon_url:clean_(r[COL.FILE_URL - 1]),
    premier_jour_statut:
      clean_(r[COL.PREMIER_JOUR_STATUT - 1]) || 'A_FAIRE',
    premier_jour_date:
      formatDateForClient_(r[COL.PREMIER_JOUR_DATE - 1]),
    premier_jour_modifie_le:
...
function sourceSheet_() {
  const sh = SpreadsheetApp
    .openById(CONFIG.SOURCE_ID)
    .getSheetByName(CONFIG.SOURCE_SHEET);
...
function checkKey_(key) {
  ...
  if (!received || !safeSecretEquals_(received, expected)) {
    throw new Error('ACCES_REFUSE');
  }
}
...
function clean_(value) {
  ...
  if (value instanceof Date) {
    return Utilities.formatDate(value, 'Europe/Paris', 'yyyy-MM-dd');
  }
...
function formatDateForClient_(value) {
  if (!value) return '';
  return value instanceof Date
    ? Utilities.formatDate(value, 'Europe/Paris', 'yyyy-MM-dd')
    : clean_(value);
}
...
function errorPayload_(err) {
  return {
    ok:false,
    code:String(err && err.message ? err.message : err),
